...
def main():
        ...
        if st.session_state.processing and video_source is not None:
            ...
            try:
                ...
                for frame, timestamp in video_frame_generator(cap, max_fps):
                    ...
                    # 檢查是否需要更新UI (每隔一定時間)
                    current_time = time.time()
                    time_since_last_update = current_time - st.session_state.current_stats["last_update_time"]
                    
                    # 減少UI更新間隔，提高響應速度
                    if (time_since_last_update >= 0.5 and  # 降低為0.5秒，提高響應性
                        st.session_state.current_stats["frame_count"] > 5):  # 減少所需幀數，更快開始更新
                        
                        # 記錄最後更新時間
                        st.session_state.current_stats["last_update_time"] = current_time
                        ...
                        # 保存當前幀位置 - 修正：在設置幀位置時加上一個較小的偏移，減少跳幀感
                        next_frame_position = current_frame_pos + 1  # 減少偏移量，避免跳幀太多
                        st.session_state.video_state["frame_position"] = next_frame_position
                        
                        # 關鍵：确保統計數據持續保存至會話狀態，這樣統計面板才能更新
                        # 即時更新顯示數據
                        st.session_state.current_stats = {
                        ...
                        # 即時統計核心機制：定期使用rerun中斷處理循環並更新UI
                        if "last_rerun_time" not in st.session_state:
                            st.session_state.last_rerun_time = 0
                            
                        # 減少rerun間隔至0.5秒，提高UI更新頻率
                        if current_time - st.session_state.last_rerun_time >= 0.5:
                            # 顯示臨時信息
                            placeholder_for_rerun = st.empty()
                            # 保存處理狀態，確保rerun後能繼續處理
                            st.session_state.temp_processing_state = True
                            st.session_state.last_rerun_time = current_time
                            logger.info("觸發UI更新，當前幀位置: " + str(next_frame_position))
                            # 關鍵：執行rerun強制更新UI
                            st.rerun()
    ...
    with col2:
        ...
        # 使用當前幀的統計數據，而不是歷史記錄的最後一項 - 實時顯示的關鍵
        # 直接從session_state讀取實時統計，而非從歷史記錄取值
        # 這樣每次rerun更新UI時都能獲取最新數據
        ...
        # 根據是否啟用追蹤，顯示不同的統計信息卡片
        if tracking_enabled:
            # 追蹤模式：顯示4個指標，包括追蹤ID數量和幀計數
            st.markdown(
                """
            <div style="display: flex; flex-direction: column; gap: 8px; margin-bottom: 16px;">
                <div style="padding: 8px; border-radius: 8px; background-color: #1e3a8a; color: white;">
                    <h5 style="margin:0; font-size: 0.85rem;">檢測人數</h5>
                    <h3 style="margin:0; font-weight: bold;">{0}</h3>
                </div>
                <div style="padding: 8px; border-radius: 8px; background-color: #065f46; color: white;">
                    <h5 style="margin:0; font-size: 0.85rem;">追蹤ID數量</h5>
                    <h3 style="margin:0; font-weight: bold;">{1}</h3>
                </div>
                <div style="padding: 8px; border-radius: 8px; background-color: #9d174d; color: white;">
                    <h5 style="margin:0; font-size: 0.85rem;">FPS</h5>
                    <h3 style="margin:0; font-weight: bold;">{2:.1f}</h3>
                </div>
                <div style="padding: 8px; border-radius: 8px; background-color: #4338ca; color: white;">
                    <h5 style="margin:0; font-size: 0.85rem;">已處理幀數</h5>
                    <h3 style="margin:0; font-weight: bold;">{3}</h3>
                </div>
            </div>
            """.format(current_count, current_tracks, current_fps, frame_count),
                unsafe_allow_html=True,
            )
        else:
            # 非追蹤模式：顯示3個指標，不包括追蹤ID數量
            st.markdown(
                """
            <div style="display: flex; flex-direction: column; gap: 8px; margin-bottom: 16px;">
                <div style="padding: 8px; border-radius: 8px; background-color: #1e3a8a; color: white;">
                ...
                <div style="padding: 8px; border-radius: 8px; background-color: #9d174d; color: white;">
                    <h5 style="margin:0; font-size: 0.85rem;">FPS</h5>
                    <h3 style="margin:0; font-weight: bold;">{1:.1f}</h3>
                </div>
                <div style="padding: 8px; border-radius: 8px; background-color: #4338ca; color: white;">
                    <h5 style="margin:0; font-size: 0.85rem;">已處理幀數</h5>
                    <h3 style="margin:0; font-weight: bold;">{2}</h3>
                </div>
            </div>
            """.format(current_count, current_fps, frame_count),
                unsafe_allow_html=True,
            )
        ...
        if st.session_state.stats["timestamps"]:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    name="人數",
                    line=dict(color="#1e40af", width=2),  # 使用更飽和的藍色
                )
            )
            ...
            fig.update_layout(
                xaxis_title="時間 (秒)",
                yaxis_title="數量",
                height=200,  # 降低高度以節省空間
                margin=dict(l=0, r=0, t=10, b=0),
                legend=dict(
                    orientation="h",  # 水平方向排列圖例
                    yanchor="bottom",
                    y=1.02,
                    xanchor="right",
                    x=1,
                ),
                font=dict(size=10),  # 減小字體大小
            )
        ...
        if st.session_state.stats["timestamps"]:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    name="FPS",
                    line=dict(color="#047857", width=2),  # 更飽和的綠色
                )
            )
        ...
        # 追蹤分析（如果啟用追蹤）
        if tracking_enabled and len(st.session_state.all_tracks) > 0:
            st.subheader("軌跡分析")
